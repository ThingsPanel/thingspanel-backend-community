<template>
  ...
  <div class="chart-out-eE2bJ0">
    <!-- 总盒子（总盒子样式勿要修改） -->
    <div class="chart-all-eE2bJ0">
      <!-- 标题盒子（每个图表必须有标题）格式和样式请勿修改 -->
      <div class="chart-top-eE2bJ0">
        <div
          style="
            text-align:center;
            color: #fff
      ...
      <!-- 图表主盒子（主盒子样式勿要修改） -->
      <div class="chart-body-eE2bJ0" :id="'chart_' + id"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    apiData: {
      ...
      immediate: true,
      handler(val, oldVal) {
        console.log("01-eE2bJ0-图表接收到数据");
        console.log("02-eE2bJ0-图表id:" + this.id);
        if (val["fields"]) {
          console.log("03-eE2bJ0-fields有值");
          console.log("04-eE2bJ0-device_id:" + val["device_id"]);
          this.latest = val["latest"];
          this.fields = val["fields"];
          this.getData();
        } else {
          console.log("05-eE2bJ0-fields没有值");
        }
      },
    },
    colorStart() {},
