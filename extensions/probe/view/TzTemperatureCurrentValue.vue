<template>
  ...
  <div class="chart-out-jD2fF8">
    <!-- 总盒子（总盒子样式勿要修改） -->
    <div class="chart-all-jD2fF8">
      <!-- 标题盒子（每个图表必须有标题）格式和样式请勿修改 -->
      <div class="chart-top-jD2fF8">
        <div
          style="
            text-align:center;
            color: #fff
      ...
      <!-- 图表主盒子（主盒子样式勿要修改） -->
      <div class="chart-body-jD2fF8" :id="'chart_' + id"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    apiData: {
      ...
      immediate: true,
      handler(val, oldVal) {
        console.log("01-jD2fF8-图表接收到数据");
        console.log("02-jD2fF8-图表id:" + this.id);
        if (val["fields"]) {
          console.log("03-jD2fF8-fields有值");
          console.log("04-jD2fF8-device_id:" + val["device_id"]);
          this.latest = val["latest"];
          this.fields = val["fields"];
          this.getData();
        } else {
          console.log("05-jD2fF8-fields没有值");
        }
      },
    },
    colorStart() {},
