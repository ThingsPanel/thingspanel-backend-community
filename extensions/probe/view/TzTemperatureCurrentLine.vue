<template>
  ...
  <div class="chart-out-kN1fF4">
    <!-- 总盒子（总盒子样式勿要修改） -->
    <div class="chart-all-kN1fF4">
      <!-- 标题盒子（每个图表必须有标题）格式和样式请勿修改 -->
      <div class="chart-top-kN1fF4">
        <div
          style="
            text-align:center;
            color: #fff
      ...
      <!-- 图表主盒子（主盒子样式勿要修改） -->
      <div class="chart-body-kN1fF4" :id="'chart_' + id"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    apiData: {
      ...
      immediate: true,
      handler(val, oldVal) {
        console.log("01-kN1fF4-图表接收到数据");
        console.log("02-kN1fF4-图表id:" + this.id);
        if (val["fields"]) {
          console.log("03-kN1fF4-fields有值");
          console.log("04-kN1fF4-device_id:" + val["device_id"]);
          this.latest = val["latest"];
          this.fields = val["fields"];
          this.getData();
        } else {
          console.log("05-kN1fF4-fields没有值");
        }
      },
    },
    colorStart() {},
  ...
  methods: {
    getData() {
      console.log("kN1fF4 进入apiData处理");
      // 最后再刷新数据
      //判断是否是第一次请求来的数据
      if (this.flag == 0) {
        console.log("kN1fF4 第一次apiData处理开始");
        this.flag = 1;
        //遍历数据字典，获取曲线数据
        ...
        console.log("kN1fF4 第一次apiData处理完成");
      } else {
        console.log("kN1fF4 apiData后续处理");
        //遍历数据字典，获取曲线数据
        for (var i = 0; i < this.fields.length; i++) {
          var item = this.fields[i];
          var d = new Date(item["systime"]);
