<template>
  ...
  <div class="chart-out-aB7aJ6">
    <!-- 总盒子（总盒子样式勿要修改） -->
    <div class="chart-all-aB7aJ6">
      <!-- 标题盒子（每个图表必须有标题）格式和样式请勿修改 -->
      <div class="chart-top-aB7aJ6">
        <div style="
            text-align:center;
            color: #fff;
            width: 100%;;
      ...
      <!-- 图表主盒子（主盒子样式勿要修改） -->
      <div class="chart-body-aB7aJ6" :id="'chart_' + id"></div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    watch: {
      apiData: {
        ...
        immediate: true,
        handler(val, oldVal) {
          console.log("aB7aJ6 接收到apiData");
          console.log("aB7aJ6 id:" + this.id);
          if (val["fields"]) {
            console.log("aB7aJ6 fields有值:");
            console.log(
              "aB7aJ6 device_id:" + val["device_id"]
            );
            this.latest = val["latest"];
            this.fields = val["fields"];
            this.getData();
          } else {
            console.log("aB7aJ6 fields没有值:");
          }
        },
      },
      colorStart() { },
    ...
    methods: {
      // 插件调试区域03
      getData() {
        console.log("aB7aJ6 进入apiData处理");
        // 最后再刷新数据
        //判断是否是第一次请求来的数据
        if (this.flag == 0) {
          console.log("aB7aJ6 第一次apiData处理开始");
          this.flag = 1;
          //遍历数据字典，获取曲线数据
          for (var i = 0; i < this.fields.length; i++) {
            ...
            if (d >= this.mytime) {
              ...
              this.carbon.push(item["carbon"]);
              this.sysTimeData.push(item["systime"].slice(11, 16));
            }
          }
          console.log("aB7aJ6 第一次apiData处理完成");
        } else {
          console.log("aB7aJ6 apiData后续处理");
          //遍历数据字典，获取曲线数据
          for (var i = 0; i < this.fields.length; i++) {
            var item = this.fields[i];
            var d = new Date(item["systime"]);
