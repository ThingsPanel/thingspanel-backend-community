<template>
  ...
  <!-- 所有class后必须跟随机密码，打开上面网页勾掉特殊字符生成随机密码如下nO5lF1 -->
  <div class="chart-out-nO5lF1">
    <!-- 总盒子（总盒子样式勿要修改） -->
    <div class="chart-all-nO5lF1">
      <!-- 标题盒子（每个图表必须有标题）格式和样式请勿修改 -->
      <div class="chart-top-nO5lF1">
        <div
          style="
            text-align:center;
            color: #fff;
      ...
      <!-- 图表主盒子（主盒子样式勿要修改） -->
      <div class="chart-body-nO5lF1" :id="'chart_' + id"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    apiData: {
      ...
      immediate: true,
      handler(val, oldVal) {
        console.log("01-nO5lF1-图表接收到数据");
        console.log("02-nO5lF1-图表id:" + this.id);
        if (val["fields"]) {
          console.log("03-nO5lF1-fields有值");
          console.log("04-nO5lF1-device_id:" + val["device_id"]);
          this.latest = val["latest"];
          this.fields = val["fields"];
          this.getData();
        } else {
          console.log("05-nO5lF1-fields没有值");
        }
      },
    },
    colorStart() {},
  ...
  methods: {
    // 给图表中的数据赋值
    getData() {
      this.dataOne = this.latest.hum;
      setTimeout(() => {
        this.initChart();
      }, 1000);
    },
    initChart() {
      console.log("05-nO5lF1-初始化图表开始");
