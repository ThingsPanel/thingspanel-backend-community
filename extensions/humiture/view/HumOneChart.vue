<template>
  ...
  <div class="chart-out-fT2hL1">
    <!-- 总盒子（总盒子样式勿要修改） -->
    <div class="chart-all-fT2hL1">
      <!-- 标题盒子（每个图表必须有标题）格式和样式请勿修改 -->
      <div class="chart-top-fT2hL1">
        <div style="
            text-align:center;
            color: #fff;
            width: 100%;;
      ...
      <!-- 图表主盒子（主盒子样式勿要修改） -->
      <div class="chart-body-fT2hL1" :id="'chart_' + id"></div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    watch: {
      apiData: {
        ...
        immediate: true,
        handler(val, oldVal) {
          console.log("fT2hL1 接收到apiData");
          console.log("fT2hL1 id:" + this.id);
          if (val["fields"]) {
            console.log("fT2hL1 fields有值:");
            console.log(
              "fT2hL1 device_id:" + val["device_id"]
            );
            this.latest = val["latest"];
            this.fields = val["fields"];
            this.getData();
          } else {
            console.log("fT2hL1 fields没有值:");
          }
        },
      },
      colorStart() { },
    ...
    methods: {
      // 插件调试区域03
      getData() {
        console.log("fT2hL1 进入apiData处理");
        // 最后再刷新数据
        //判断是否是第一次请求来的数据
        if (this.flag == 0) {
          console.log("fT2hL1 第一次apiData处理开始");
          this.flag = 1;
          //遍历数据字典，获取曲线数据
          ...
          console.log("fT2hL1 第一次apiData处理完成");
        } else {
          console.log("fT2hL1 apiData后续处理");
          //遍历数据字典，获取曲线数据
          for (var i = 0; i < this.fields.length; i++) {
            var item = this.fields[i];
            var d = new Date(item["systime"]);
