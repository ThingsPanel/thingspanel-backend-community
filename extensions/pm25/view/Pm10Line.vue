<template>
  ...
  <div class="chart-out-bG8jB3">
    <!-- 总盒子（总盒子样式勿要修改） -->
    <div class="chart-all-bG8jB3">
      <!-- 标题盒子（每个图表必须有标题）格式和样式请勿修改 -->
      <div class="chart-top-bG8jB3">
        <div style="
            text-align:center;
            color: #fff;
            width: 100%;;
      ...
      <!-- 图表主盒子（主盒子样式勿要修改） -->
      <div class="chart-body-bG8jB3" :id="'chart_' + id"></div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    watch: {
      apiData: {
        ...
        immediate: true,
        handler(val, oldVal) {
          console.log("bG8jB3 接收到apiData");
          console.log("bG8jB3 id:" + this.id);
          if (val["fields"]) {
            console.log("bG8jB3 fields有值:");
            console.log(
              "bG8jB3 device_id:" + val["device_id"]
            );
            this.latest = val["latest"];
            this.fields = val["fields"];
            this.getData();
          } else {
            console.log("bG8jB3 fields没有值:");
          }
        },
      },
      colorStart() { },
    ...
    methods: {
      // 插件调试区域03
      getData() {
        console.log("bG8jB3 进入apiData处理");
        // 最后再刷新数据
        //判断是否是第一次请求来的数据
        if (this.flag == 0) {
          console.log("bG8jB3 第一次apiData处理开始");
          this.flag = 1;
          //遍历数据字典，获取曲线数据
          ...
          console.log("bG8jB3 第一次apiData处理完成");
        } else {
          console.log("bG8jB3 apiData后续处理");
          //遍历数据字典，获取曲线数据
          for (var i = 0; i < this.fields.length; i++) {
            var item = this.fields[i];
            var d = new Date(item["systime"]);
