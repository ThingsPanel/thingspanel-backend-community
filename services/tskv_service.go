...
func (*TSKVService) Paginate(business_id, asset_id, token string, t_type int64, start_time string, end_time string, limit int, offset int, key string, device_name string) ([]models.TSKVDblV, int64) {
	...
	if device_name != "" { //key
		SQLWhere = SQLWhere + ` and device."name" like ?`
...
func (*TSKVService) GetHistoryData(device_id string, attributes []string, startTs int64, endTs int64, rate string) map[string][]interface{} {
	...
	if rate == "" {
		...
		sqlQuery := "select key, bool_v, str_v, long_v, dbl_v, ts from (select row_number() over " +
			fmt.Sprintf("(partition by (times,key) order by ts,key desc) as seq,* from (select tk.ts/%s", rate) + " as times ,* from ts_kv tk where" +
			fmt.Sprintf(" ts >= '%d' AND ts <= '%d' AND entity_id ='%s' AND key in (%s)) as tks) as group_tk where seq = 1", startTs*1000, endTs*1000, device_id, strings.Join(attributes, ","))
		result = psql.Mydb.Raw(sqlQuery).Find(&ts_kvs)
	}
	if result.Error != nil {
		errors.Is(result.Error, gorm.ErrRecordNotFound)
		return rsp_map
