...
type OpenapiTSKVService struct {
	//可搜索字段
	SearchField []string
	//可作为条件的字段
	WhereField []string
...
func (*OpenapiTSKVService) All(tenantId string) (int64, error) {
	var count int64
...
// 接收硬件其他消息（在线离线）
func (*OpenapiTSKVService) MsgProcOther(body []byte, topic string) {
	logs.Info("-------------------------------")
...
// 接收网关消息
func (s *OpenapiTSKVService) GatewayMsgProc(body []byte, topic string) bool {
	logs.Info("------------------------------")
	logs.Info("来自网关设备的消息：")
	logs.Info(string(body))
	logs.Info("------------------------------")
	...
	for _, sub_device := range sub_device_list {
		if values, ok := payload_map[sub_device.SubDeviceAddr]; ok {
			...
			if err != nil {
				...
				s.MsgProc(sub_payload_bytes, topic)
			}
		}
	}
	return true
...
// 接收硬件消息 ，数据转发切入点，（设备上报）
func (*OpenapiTSKVService) MsgProc(body []byte, topic string) bool {
	logs.Info("-------------------------------")
...
// 分页查询数据
func (*OpenapiTSKVService) Paginate(business_id, asset_id, token string, t_type int64, start_time string, end_time string, limit int, offset int, key string, device_name string, tenant_id string) ([]models.TSKVDblV, int64) {
...
// 通过设备ID获取一段时间的数据
func (*OpenapiTSKVService) GetTelemetry(device_ids []string, startTs int64, endTs int64, rate string) []interface{} {
...
// 通过设备ID获取一段时间的数据
func (*OpenapiTSKVService) GetHistoryData(device_id string, attributes []string, startTs int64, endTs int64, rate string) map[string][]interface{} {
	var ts_kvs []models.TSKV
	var result *gorm.DB
...
func (*OpenapiTSKVService) Status(device_id string) (*models.TSKVLatest, int64) {
...
// 通过设备ID获取设备当前值
func (*OpenapiTSKVService) GetCurrentData(device_id string, attributes []string) []map[string]interface{} {
	var fields []map[string]interface{}
...
func (s *OpenapiTSKVService) GetCurrentDataByBusiness(ctx *context2.Context, business string) map[string]interface{} {
	var DeviceService services.DeviceService
	deviceList, deviceCount := DeviceService.GetDevicesByBusinessID(business)
	//判断设备访问权限
	isAll, accessDevices := s.GetAccessDevices(ctx, deviceList)
	if !isAll {
		deviceList = accessDevices
	}
	...
	if len(deviceList) != 0 {
		for _, device := range deviceList {
			...
			fields := s.GetCurrentData(device.ID, nil)
			if len(fields) == 0 {
...
func (s *OpenapiTSKVService) GetCurrentDataByAsset(ctx *context2.Context, asset_id string) map[string]interface{} {
	var DeviceService services.DeviceService
...
func (s *OpenapiTSKVService) GetCurrentDataByAssetA(ctx *context2.Context, asset_id string) map[string]interface{} {
	var DeviceService services.DeviceService
	deviceList, deviceCount := DeviceService.GetDevicesInfoAndCurrentByAssetID(asset_id)
	//判断设备访问权限
	isAll, accessDevices := s.GetAccessDevices(ctx, deviceList)
...
func (*OpenapiTSKVService) DeviceHistoryData(device_id string, current int, size int) ([]map[string]interface{}, int64) {
	var ts_kvs []models.TSKV
	var count int64
...
// 设备在线离线判断
func (*OpenapiTSKVService) DeviceOnline(device_id string, interval int64) (string, error) {
	var ts_kvs models.TSKVLatest
