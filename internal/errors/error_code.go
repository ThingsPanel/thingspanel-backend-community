...
var errorCodeMap = map[int]ErrorCode{
	ErrSystemInternal: {ErrSystemInternal, "内部系统错误", http.StatusBadRequest},
	ErrDatabaseError:  {ErrDatabaseError, "数据库操作错误", http.StatusBadRequest},
	ErrNetworkError:   {ErrNetworkError, "网络通信错误", http.StatusBadRequest},
	ErrConfigError:    {ErrConfigError, "系统配置错误", http.StatusBadRequest},
	ErrCacheError:     {ErrCacheError, "缓存操作错误", http.StatusBadRequest},
	...
	ErrAccountLocked:         {ErrAccountLocked, "账户已锁定", http.StatusBadRequest},
	ErrTooManyAttempts:       {ErrTooManyAttempts, "尝试次数过多", http.StatusBadRequest},
	ErrPasswordExpired:       {ErrPasswordExpired, "密码已过期", http.StatusBadRequest},
	ErrUserNotFound:          {ErrUserNotFound, "用户不存在", http.StatusBadRequest},
	ErrInvalidCredentials:    {ErrInvalidCredentials, "用户名或密码错误", http.StatusBadRequest},
	ErrUserDisabled:          {ErrUserDisabled, "用户状态异常", http.StatusBadRequest},
	...
	ErrResourceNotFound:   {ErrResourceNotFound, "资源未找到", http.StatusBadRequest},
	ErrDuplicateEntry:     {ErrDuplicateEntry, "重复条目", http.StatusBadRequest},
	ErrOperationFailed:    {ErrOperationFailed, "操作失败", http.StatusBadRequest},
	ErrDeviceNotFound:     {ErrDeviceNotFound, "设备未找到", http.StatusBadRequest},
	ErrDeviceOffline:      {ErrDeviceOffline, "设备离线", http.StatusBadRequest},
	ErrDeviceUnauthorized: {ErrDeviceUnauthorized, "设备未授权", http.StatusBadRequest},

	ErrFileUpload:      {ErrFileUpload, "文件上传错误", http.StatusBadRequest},
	ErrDataProcessing:  {ErrDataProcessing, "数据处理错误", http.StatusBadRequest},
	ErrFileTooLarge:    {ErrFileTooLarge, "文件过大", http.StatusBadRequest},
