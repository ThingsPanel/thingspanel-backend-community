...
var errorCodeMap = map[int]ErrorCode{
	...
	ErrAccountLocked:         {ErrAccountLocked, "账户已锁定", http.StatusOK},
	ErrTooManyAttempts:       {ErrTooManyAttempts, "尝试次数过多", http.StatusOK},
	ErrPasswordExpired:       {ErrPasswordExpired, "密码已过期", http.StatusOK},
	ErrUserNotFound:          {ErrUserNotFound, "用户不存在", http.StatusOK},
	ErrInvalidCredentials:    {ErrInvalidCredentials, "用户名或密码错误", http.StatusOK},
	ErrUserDisabled:          {ErrUserDisabled, "用户状态异常", http.StatusOK},
