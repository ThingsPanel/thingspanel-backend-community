...
var errorCodeMap = map[int]ErrorCode{
	...
	ErrNetworkError:   {ErrNetworkError, "网络通信错误", http.StatusInternalServerError},
	...
	ErrAccountLocked:         {ErrAccountLocked, "账户已锁定", http.StatusInternalServerError},
	ErrTooManyAttempts:       {ErrTooManyAttempts, "尝试次数过多", http.StatusInternalServerError},
	ErrPasswordExpired:       {ErrPasswordExpired, "密码已过期", http.StatusInternalServerError},
	ErrUserNotFound:          {ErrUserNotFound, "用户不存在", http.StatusInternalServerError},
	ErrInvalidCredentials:    {ErrInvalidCredentials, "用户名或密码错误", http.StatusInternalServerError},
	ErrUserDisabled:          {ErrUserDisabled, "用户状态异常", http.StatusInternalServerError},

	ErrInvalidInput:       {ErrInvalidInput, "无效的输入", http.StatusInternalServerError},
	ErrMissingParameter:   {ErrMissingParameter, "缺少必要参数", http.StatusInternalServerError},
	ErrInvalidFormat:      {ErrInvalidFormat, "格式错误", http.StatusInternalServerError},
	ErrResourceNotFound:   {ErrResourceNotFound, "资源未找到", http.StatusInternalServerError},
	ErrDuplicateEntry:     {ErrDuplicateEntry, "重复条目", http.StatusInternalServerError},
	ErrOperationFailed:    {ErrOperationFailed, "操作失败", http.StatusInternalServerError},
	ErrDeviceNotFound:     {ErrDeviceNotFound, "设备未找到", http.StatusInternalServerError},
	ErrDeviceOffline:      {ErrDeviceOffline, "设备离线", http.StatusInternalServerError},
	ErrDeviceUnauthorized: {ErrDeviceUnauthorized, "设备未授权", http.StatusInternalServerError},

	ErrFileUpload:      {ErrFileUpload, "文件上传错误", http.StatusInternalServerError},
	ErrDataProcessing:  {ErrDataProcessing, "数据处理错误", http.StatusInternalServerError},
	ErrFileTooLarge:    {ErrFileTooLarge, "文件过大", http.StatusInternalServerError},
	ErrInvalidFileType: {ErrInvalidFileType, "无效的文件类型", http.StatusInternalServerError},
}
