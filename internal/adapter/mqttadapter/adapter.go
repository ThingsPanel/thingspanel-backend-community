...
func (a *Adapter) HandleTelemetryMessage(payload []byte, topic string) error {
	...
	if err != nil {
		...
		if deviceID != "" {
			// 不增加 uplink_total（点位计数在 uplink 层），仅记录处理失败
	...
	// 记录诊断：消息进入处理流程（验证通过后）
	// uplink 总数改为按点位计数；成功消息的 uplink 计数将在 uplink 模块按点位累加，因此此处仅保留错误分支计数。
...
func (a *Adapter) HandleAttributeMessage(payload []byte, topic string) error {
	...
	// 记录诊断：消息进入处理流程（验证通过后）
	// uplink 点位计数将在 uplink 模块按点位累加，此处不再直接增加 uplink_total。
