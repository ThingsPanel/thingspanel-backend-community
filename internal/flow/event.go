...
// NewEventFlow 创建事件数据流处理器
func NewEventFlow(config EventFlowConfig) *EventFlow {
...
// Start 启动事件数据流处理
func (f *EventFlow) Start(messageChan <-chan *DeviceMessage) {
	f.logger.Info("EventFlow started")
	...
	go func() {
		for {
			...
			case msg, ok := <-messageChan:
				if !ok {
					f.logger.Info("EventFlow message channel closed")
					return
				}
				f.processMessage(msg)

			case <-f.ctx.Done():
				f.logger.Info("EventFlow stopped")
				return
			}
		}
	}()
...
// Stop 停止事件数据流处理
func (f *EventFlow) Stop() {
	f.cancel()
}

// processMessage 处理单条事件消息
func (f *EventFlow) processMessage(msg *DeviceMessage) {
	// 从 metadata 获取设备ID
...
func (f *EventFlow) processGatewayMessage(device *model.Device, payload []byte, originalMsg *DeviceMessage) {
...
func (f *EventFlow) processSubDevices(parentID string, subDeviceData map[string]model.EventInfo, originalMsg *DeviceMessage) {
...
func (f *EventFlow) processSubGateways(parentID string, subGatewayData map[string]*model.GatewayCommandPulish, originalMsg *DeviceMessage, depth int) {
	if depth > 5 {
...
func (f *EventFlow) processDirectDeviceEvent(device *model.Device, eventInfo *model.EventInfo, originalMsg *DeviceMessage) {
	// 1. 心跳刷新(最优先,确保设备活跃性)
...
func (f *EventFlow) refreshHeartbeat(device *model.Device) {
...
func (f *EventFlow) notifyDeviceOnline(device *model.Device) {
	// SSE通知
